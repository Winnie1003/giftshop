...
$("#StairNav li.Top").click(function(){
   ...
   $(".but img").mouseover(function(){
	 ...
	 $(".showsmall img,.showbig img").attr("src",bimg);
   });


   //鼠标滑动到遮罩层上.shade 时 ，的操作
   $(".proshow .showsmall .shade").hover(function(){
      $(".showsmall span.move,.showbig").show();//小滑块和大图展示显示
   },function(){
      $(".showsmall span.move,.showbig").hide();
   });

   //当鼠标在遮罩层上.shade 移动时，动态来获取 小滑块的位置
   $(".proshow .showsmall .shade").mousemove(function(e){
	  ...
	  var w=$(".showsmall span.move").width()/2;//获取小滑块宽度的一半
	  var h=$(".showsmall span.move").height()/2;//获取小滑块高度的一半

	  var _left=x-l-w;  //小滑块与 .shade 左边的距离
	  var _top=y-t-h;  //小滑块与 .shade 上面的距离
	

	  var bigW=$(".proshow .showsmall .shade").width()-w*2-2;  //滑块能滑动的最大宽度
      var bigH=$(".proshow .showsmall .shade").height()-h*2-2; //滑块能滑动的最大高度
		...
		//给小滑块赋值 top  left
		$(".proshow .showsmall span.move").css({"left":_left,"top":_top});
		...
		var bigLeft=($(".showbig img").width()-$(".showbig").width())*wbl;  //大图向左移动距离
		var bigTop=($(".showbig img").height()-$(".showbig").height())*hbl;  //大图向上移动距离)

		$(".showbig img").css({"left":-bigLeft,"top":-bigTop}); //动态给大图赋值
   });
